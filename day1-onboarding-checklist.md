...
- A compact top-left HUD pop-up describes the next action from actual customer, cup, carrying, and inspection state. Each distinct hint appears once per lesson for the duration configured on the Day 1 asset (six seconds by default), then disappears; returning to an old action does not replay it. New actions replace stale hints. Existing E/F prompts and conversations remain authoritative and remain visible independently.
...
6. To tune the pop-up time later, select **Assets/Data/Days/Day_01_LearnTheShop** and change **Opening Hint Duration** (3–10 seconds) outside Play Mode.
...
- [ ] Stand still: the hint disappears after about six seconds and stays hidden. The next new action (such as taking a cup) shows its own pop-up. Going back to an already shown action does not replay it.
...
- [ ] During inspection, a new pop-up appears when the next milestone changes: select screwdriver, remove the screws, select pry, lift the cover, select the fault tool (brush or tweezers), fix the fault, refit the cover, and refit the screws. Hover guidance and existing tool prompts remain readable. Reassemble, exit inspection, carry the item back and press E at Hand it back (no intake conversation needed).
